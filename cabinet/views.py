...
def bills_paid(request):
    return render(request, "cabinet/placeholder.html", {"title": "Квитанции — оплаченные"})

def bills_overdue(request):
    return render(request, "cabinet/placeholder.html", {"title": "Квитанции — просроченные"})

def tariffs_active(request):
    return render(request, "cabinet/placeholder.html", {"title": "Тарифы — действующие"})
